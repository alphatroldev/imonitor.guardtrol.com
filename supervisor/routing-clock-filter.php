...
<?php 
if (!isset($_SESSION['SUPERVISOR_LOGIN'])) header("Location: ./"); 
?>
<?php
if (empty($_POST['beat_check']) || empty($dates)){
    echo "<script>window.location = '".url_path('/supervisor/routing-clock',true,true)."';</script>";
} else {
    ?>
    <section role="main" class="mx-5">
        <div class="justify-content-center mb">
            <div  class="text-center">
                <a href="<?= url_path('/supervisor/routing-clock',true,true)?>">
                    <img src="<?=public_path('uploads/company/'.$c['company_logo']);?>" class="rounded float-left" width="80" height="80" alt="Company Logo" />
                </a>
            </div>
            <div class="px-3">
                <h3 class="text-center text-uppercase">
                    <a href="<?= url_path('/supervisor/routing-clock',true,true)?>">GUARD ROUTE REPORT</a>
                </h3>
            </div>
        </div>
        <div class="card mt-2">
            <header class="card-header">
                <h2 class="card-title text-uppercase">
                    <a style="padding-right:15px;" href="<?= url_path('/supervisor/routing-clock',true,true)?>" class="no_style">
            ...
            <div class="card-body">
                <table class="table table-responsive-md table-bordered mb-0" id="datatable-tabletools" style="font-size: 12px;">
                    ...
                    <tbody>
                    <?php
                    $res = $supervisor->get_supervisor_guard_routing_report($comp_id,$bsu_beats);
                            ...
                            if (in_array($clock_date, $all_dates)){
                                if (in_array($row['beat_id'], $_POST['beat_check'])){
                                    ...
                                    <tr>
                                        ...
                                        <td><b><?= ($row['start_time'] != "0000-00-00 00:00:00")?date("H:i:s", strtotime($row['start_time'])):'00:00:00';?></b></td>
                                        <td><b><?= ($row['end_time'] != "0000-00-00 00:00:00")?date("H:i:s", strtotime($row['end_time'])):'00:00:00';?></b></td>
    ...
    (function($) {
        ...
        var datatableInit = function() {
            ...
            var table = $table.dataTable({
                ...
                buttons: [
                    {extend: 'print', text: 'Print',title: function(){ var printTitle = 'Guard Route Report'; return printTitle}},
                    {extend: 'excel', text: 'Excel'},
                    {extend: 'pdf',orientation: 'landscape',text: 'PDF',title: function(){ var printTitle = 'Guard Route Report'; return printTitle},
