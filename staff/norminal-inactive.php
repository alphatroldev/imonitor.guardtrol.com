...
<?php if(!in_array($norminal_row_permission_sno['perm_sno'], $array))header("Location:".$location_redirect);?>

<section role="main" class="content-body">
        <header class="page-header">
            <h2>Inactive Norminal Roll</h2>
            <div class="right-wrapper text-end">
                <ol class="breadcrumbs">
                    <li><a href="index"><i class="bx bx-home-alt"></i></a></li>
                    <li><span>Inactive Norminal Roll</span></li>
                </ol>
            ...
            <div class="col">
                <section class="card">
                    <header class="card-header">
                        ...
                        <h2 class="card-title">Inactive Norminal Rolls</h2>
                    </header>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="mb-3">
                                    <a href="<?= url_path('/staff/deploy-guard',true,true)?>" class="btn btn-primary"><i class="fas fa-plus">&nbsp;&nbsp;</i>Deploy Guard</a>
                                </div>
                            </div>
                            <div class="col-sm-6">
                                <div class="mb-3 text-end">
                                    <a href="<?= url_path('/staff/list-norminal-rolls',true,true)?>" class="btn btn-success">
                                        <i class="fas fa-check-square">&nbsp;&nbsp;</i>Active norminal rolls</a>
                                </div>
                            </div>
                        </div>
                        <table class="table table-bordered table-striped mb-0" id="datatable-tabletools">
                            ...
                            <tbody>
                            <?php
                            $res = $deploy_guard->get_inactive_norminal_roll($c['company_id']);
                            if ($res->num_rows > 0) {$n=0;
                                while ($row = $res->fetch_assoc()) {
                                    ...
                                    <tr>
                                        ...
                                        <td class="actions">
                                            <a href="<?= url_path('/staff/edit-norminal-roll/'.$row['guard_id'],true,true)?>" class="on-default edit-row"><i class="fas fa-pencil-alt"></i></a>
                                            <a href="javascript:void(0)" data-id="<?= $row['gd_id'];?>" class="on-default remove-row" id="norminalRollDeleteBtn">
                                                <i class="far fa-trash-alt"></i>
                                            </a>
                                        </td>
                                    </tr>
                                <?php } } else { echo "<tr><td colspan='12' class='text-center'>No inactive deployment</td></tr>";} ?>
                            </tbody>
                        </table>
                    </div>
                </section>
