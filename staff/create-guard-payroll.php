        ...
        <div class="col-md-12">
            <section class="card card-featured card-featured-primary mb-4">
                <header class="card-header">
                    <h2 class="card-title">Generate Guard Payroll</h2>
                </header>
                <div class="card-body">
                    <form name="generate_guard_payroll" id="generate_guard_payroll" class="row">
                        <div class="col-sm-7">
                            <table class="table table-bordered table-hover mb-0" id="datatable-company">
                                ...
                                <tbody>
                                <?php
                                $res = $company->get_all_company_deployed_guard_account($c['company_id']);
                                    ...
                                    while ($row = $res->fetch_assoc()) {
                                        ...
                                        <tr>
                                            <td class="text-primary">
                                                ...
                                                <?= $row['guard_firstname']." ".$row['guard_middlename']." ".$row['guard_lastname'];?>
                                            </td>
                                            <td>
                                                <div class="checkbox-custom checkbox-default pull-right">
                                                    <input type="checkbox" class="guard_check" id="guard_check_<?= $row['guard_id']?>" name="guard_check[]" value="<?= $row['guard_id']?>">
                                                    <label for="guard_check_<?= $row['guard_id']?>"></label>
                                                </div>
                                            </td>
                                        </tr>
...
<script>
    function check_uncheck_grd(isChecked) {
        if (isChecked){
            $('input[name="guard_check[]"]').each(function(){
                this.checked = true;
