...
<?php if(!in_array($deploy_guard_permission_sno['perm_sno'], $array))header("Location:".$location_redirect);?>
                            ...
                            <div class="form-group row pb-3">
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <label class="control-label pt-1" for="w1-guard">Guard <span class="required">*</span></label>
                                        <select data-plugin-selectTwo class="form-control populate placeholder" data-plugin-options='{ "placeholder": "Select Guard", "allowClear": true }' name="guard_id" id="w1-guard" required>
                                            <option value=""></option>
                                            <?php
                                            $all_guards = $guard->get_all_company_guards($c['company_id']);
                                            while ($row_guard = $all_guards->fetch_assoc()) {
                                                $deployed_guard = $deploy_guard->get_deployed_guard($row_guard['guard_id']);
                                                $check_guard = $deployed_guard->fetch_assoc();

                                                if($check_guard['guard_id']!==$row_guard['guard_id']){
                                                    ?>
                                                    <option value="<?=$row_guard['guard_id'];?>"><?= $row_guard['guard_firstname']." ".$row_guard['guard_middlename']." ".$row_guard['guard_lastname'];?></option>
                                                <?php } }  ?>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <label class="control-label pt-1" for="w1-beat">Beat <span class="required">*</span></label>
