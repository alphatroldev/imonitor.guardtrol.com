...
<section role="main" class="mx-5">
    <div class="d-flex justify-content-center mb">
        <div>
            <a href="<?= url_path('/staff/beat-guard-payroll-history',true,true)?>">
                <img src="<?=public_path('uploads/company/'.$c['company_logo']);?>" class="rounded float-left" width="80" height="80" alt="Company Logo" />
            </a>
        </div>
        <div class="px-3">
            <h3 class="text-center text-uppercase">
                <a href="<?= url_path('/staff/beat-guard-payroll-history',true,true)?>"><?=$c['company_name'];?> GUARD PAYROLL </a>
            </h3>
        </div>
    </div>
    <div class="card mt-2">
        ...
        <div class="card-body">
            ...
            while ($row = $res->fetch_assoc()) {
                echo "<span class='font-weight-bold'>".$row['client_fullname']."</span>";
                $res2 = $beat->get_all_beats($c['company_id'],$row['client_id']);
                while ($row2 = $res2->fetch_assoc()) {
                    echo "<p class='pt-3'>".$row2['beat_name']."</p>";
            ?>
            <table class="table table-responsive-md table-bordered mb-3" style="font-size: 9px;">
                <thead>
                <tr>
                    <th></th>
                    <th></th>
                ...
                <tbody>
                <?php
                $res_G = $company->get_beat_guard_payroll_history_details($c['company_id'],$row['client_id'],$row2['beat_id'],$mon,$year);
                if ($res_G->num_rows > 0) { $n=0;
                    while ($gpr = $res_G->fetch_assoc()) {
                        ...
                        <tr>
                            ...
                            <td><?=($tot_re_amount==0)?0:'<a target="_blank" href="'.url_path("/staff/guard-redeployment-details/".$gpr['guard_id']."/".$mon_year,true,true).'">₦'.number_format($tot_re_amount,0).'</a>';?></td>
