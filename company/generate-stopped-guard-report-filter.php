...
if ($from_month=='' || $from_year=='' || $to_month=='' || $to_year==''){
    echo "<script>window.location = '".url_path('/company/generate-stopped-guard-report',true,true)."';</script>";
} else {
    ...
    <section role="main" class="mx-5">
        <div class="d-flex justify-content-center mb">
            <div>
                <a href="<?= url_path('/company/generate-stopped-guard-report',true,true)?>">
            ...
            <div class="px-3">
                <h3 class="text-center text-uppercase">
                    <a href="<?= url_path('/company/generate-stopped-guard-report',true,true)?>">
                        <?=$c['company_name'];?> GUARD STOPPED/AWOL REPORT
                    </a>
                </h3>
            </div>
        </div>
        <div class="card mt-2">
            <header class="card-header">
                <a style="float:left;" href="<?= url_path('/company/generate-stopped-guard-report',true,true)?>" class="no_style">
            ...
            <div class="card-body">
                <table class="table table-responsive-md table-bordered mb-0" id="datatable-tabletools" style="font-size: 12px;">
                    ...
                    <tbody>
                    <?php
                    $g_all_in_guard = $company->get_inactive_guards($c['company_id']);
                    if ($g_all_in_guard->num_rows > 0) { $n=0;
                    while ($row_in_guard = $g_all_in_guard->fetch_assoc()) {
                        if (!empty($res = $company->fetch_guard_status_table($c['company_id'],$row_in_guard['guard_id']))){
                        $mon_year = date("Y-m",strtotime($res['stat_date']));
                        if (in_array($mon_year, $all_months)){
//                        if ($res['guard_status']=="Deactivate"){
                    ?>
                        <tr>
                            <td><?=++$n;?></td>
                            <td><?=$res['guard_firstname']." ".$res['guard_middlename']." ".$res['guard_lastname'];?></td>
                            <td><?=date("d/m/Y",strtotime($res['stat_date']));?></b></td>
                            <td><?=$res['beat_name'];?></td>
                            <td><?=$res['remark'];?></td>
                        </tr>
                    <?php } } } }  ?>
