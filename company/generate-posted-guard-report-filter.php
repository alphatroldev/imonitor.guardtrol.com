...
if ($from_month=='' || $from_year=='' || $to_month=='' || $to_year==''){
    echo "<script>window.location = '".url_path('/company/generate-posted-guard-report',true,true)."';</script>";
} else {
    ...
    <section role="main" class="mx-5">
        <div class="d-flex justify-content-center mb">
            <div>
                <a href="<?= url_path('/company/generate-posted-guard-report',true,true)?>">
            ...
            <div class="px-3">
                <h3 class="text-center text-uppercase">
                    <a href="<?= url_path('/company/generate-posted-guard-report',true,true)?>">
                        <?=$c['company_name'];?> NEWLY POSTED GUARD REPORT
                    </a>
                </h3>
            </div>
        </div>
        <div class="card mt-2">
            <header class="card-header">
                <a style="float:left;" href="<?= url_path('/company/generate-posted-guard-report',true,true)?>" class="no_style">
            ...
            <div class="card-body">
                <table class="table table-responsive-md table-bordered mb-0" id="datatable-tabletools" style="font-size: 12px;">
                    ...
                    <tbody>
                    <?php
                    $res = $company->get_newly_posted_guard_report($c['company_id']);
                    if ($res->num_rows > 0) { $n=0;
                        while ($row = $res->fetch_assoc()) {
                            $mon_year = date("Y-m",strtotime($row['gd_date']));
                            if (in_array($mon_year, $all_months)){
                                ?>
                                <tr>
                                    <td><?=++$n;?></td>
                                    <td><?=date("d/m/Y",strtotime($row['dop']));?></td>
                                    <td><?=date("d/m/Y",strtotime($row['observation_start']));?></td>
                                    <td><?=date("d/m/Y",strtotime($row['commencement_date']));?></td>
