...
if ($from_month=='' || $from_year=='' || $to_month=='' || $to_year==''){
    echo "<script>window.location = '".url_path('/company/generate-guard-extra-duty-report',true,true)."';</script>";
} else {
    ...
    <section role="main" class="mx-5">
        <div class="d-flex justify-content-center mb">
            <div>
                <a href="<?= url_path('/company/generate-guard-extra-duty-report',true,true)?>">
            ...
            <div class="px-3">
                <h3 class="text-center text-uppercase">
                    <a href="<?= url_path('/company/generate-guard-extra-duty-report',true,true)?>">
                        <?=$c['company_name'];?> GUARD EXTRA DUTIES REPORT
                    </a>
                </h3>
            </div>
        </div>
        <div class="card mt-2">
            <header class="card-header">
                <a style="float: left;" href="<?= url_path('/company/generate-guard-extra-duty-report',true,true)?>" class="no_style">
            ...
            <div class="card-body">
                <table class="table table-responsive-md table-bordered mb-0" id="datatable-tabletools" style="font-size: 12px;">
                    ...
                    <tbody>
                    <?php
                    $res = $company->get_guard_extra_duty_report($c['company_id']);
                    if ($res->num_rows > 0) { $n=0;
                        while ($row = $res->fetch_assoc()) {
                            $mon_year = date("Y-m",strtotime($row['exd_date']));
                            if (in_array($mon_year, $all_months)){
                                $guard = $company->get_guard_name_for_extra_duty($c['company_id'],$row['guard_id']);
                                $guard_name = $guard['full_name'];
                                $guard_replace = $company->get_guard_name_for_extra_duty($c['company_id'],$row['guard_replace']);
                                $guard_replace_name = !empty($guard_replace['full_name'])?$guard_replace['full_name']:"Short Legs";
                                ?>
                                <tr>
                                    <td><?=++$n;?></td>
                                    <td><?=$guard_name;?></td>
                                    <td><?=$row['beat_name'];?></td>
                                    <td><b><?=date("d/m/Y",strtotime($row['exd_date']));?></b></td>
                                    <td><?=$row['no_of_days'];?> <?= $row['no_of_days']!=0? 'Days':'';?></td>
                                    <td>₦<?= number_format($row['days_amount'],0);?></td>
                                    <td><?=$guard_replace_name;?></td>
                                    <td><?=$row['reason_remark'];?></td>
                                </tr>
                            <?php } } } ?>
                    </tbody>
                </table>
