...
<?php
if (!isset($beat_id) ||$beat_id == NULL ) {echo "<script>window.location = '".url_path('/company/list-beats',true,true)."'; </script>";}
?>
<?php
            ...
            <table style="font-size:11px;" id="datatable-activity" class="table table-striped table-no-more table-bordered mb-0">
                ...
                <tr>
                    ...
                    <td data-title="Message" class="pt-3 pb-3">
                        <?=$row['action_title'];?> - <?=$row['action_message'];?> <br>
                        <small style="font-style:italic;"><?=($row['action_reason']!='')?'PS: '.$row['action_reason']:'';?></small>
                    </td>
                </tr>
                <?php } } ?>
                </tbody>
