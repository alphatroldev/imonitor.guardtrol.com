...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    if (!empty($c_agent) && !empty($c_uniform_charge) && !empty($c_act_date) && !empty($loan_type)) {
        if($c_agent == 'Yes' && ($c_agent_fee == null || $c_agent_fee == 0)){
            ...
            $c_data = $company->update_company_config($com_id, $c_uniform_charge,$c_uniform_charge_amt,$c_uni_mode,$c_agent ,$c_agent_fee,$c_agent_mode,$c_shift,$c_act_date,$loan_type,$c_penalties);
            if ($c_data) {
                http_response_code(200);
                echo json_encode(array("status" => 1, "message" => "Configuration has been successfully saved."));
            } else {
                http_response_code(200);
                echo json_encode(array("status" => 0, "message" => "Server error, could not save configuration"));
            }
        }
    } else {
        http_response_code(200);
